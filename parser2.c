...
void main3(int printParser)
{
	...
	while(fscanf(file, "%d", &TOKEN) == 1)
	{
		...
		if(TOKEN == 3)
		{
			...
			//fscanf(file, "%d", numList[numNum]);
//			printf()
			//printf( "%s 1", variableList[VarNum]);
			//fprintf(fs, "%d ", numList[numNum]);
			numNum++;
		}
		j++;
    }
...
void STATEMENT()
{
	...
	else if (TOKEN == ifsym)
	{
		...
		//sprintf(PrintmaStack[pc], "14 %d %d %d\n", curReg, curReg, (curReg+1));//subtract 1 so the answer should be at 0
			pc++;
            sprintf(PrintmaStack[pc], "8 0 0 %d\n",  (pc+2));//jump past the jump that skips this part of the code
			pc++;
			tempPC2=pc;
